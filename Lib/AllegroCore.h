...
character jogador[8];
//INICIAR SPRITES
ALLEGRO_BITMAP *spriteEnemy[4],*spriteEnemyHit[4],*spriteGuard[4],*spriteGuardHit[4], *spriteShield = NULL;
...
//Imagens
ALLEGRO_BITMAP *map = NULL, *logocin = NULL;
// A nossa janela
...
//  spriteEnemyHit[3] = al_load_bitmap("Sprites/enemyGreenHit.png");
    ...
    al_convert_mask_to_alpha(spriteGuard[0],al_map_rgb(0,64,88));
    al_convert_mask_to_alpha(spriteGuard[2],al_map_rgb(0,64,88));
    al_convert_mask_to_alpha(spriteGuardHit[0],al_map_rgb(0,64,88));
    al_convert_mask_to_alpha(spriteGuardHit[2],al_map_rgb(0,64,88));
...
int downloadFonts(){
    ...
    fontesecundaria = al_load_font("Fonts/SuperMario256.ttf", 42, 0);
    if (!fontesecundaria){
        al_destroy_display(janela);
        fprintf(stderr, "Falha ao carregar fonte.\n");
        return 0;
